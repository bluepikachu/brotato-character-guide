# Brotato 中英术语对照

用于把不同来源里的英文角色、武器、道具、属性统一成中文输出。

## 角色

- Well Rounded = 全能者
- Brawler = 斗士
- Crazy = 狂人
- Ranger = 游侠
- Mage = 法师
- Chunky = 大壮
- Old = 老叟
- Lucky = 幸运星
- Mutant = 异变体
- Generalist = 博学家
- Loud = 大嗓门
- Multitasker = 多面手
- Wildling = 野人
- Pacifist = 和平主义者
- Gladiator = 角斗士
- Saver = 节俭者
- Sick = 病人
- Farmer = 农夫
- Ghost = 幽灵
- Speedy = 飞毛腿
- Entrepreneur = 企业家
- Engineer = 工程师
- Explorer = 探险家
- Doctor = 医生
- Hunter = 猎人
- Technomage = 科技法师
- Arms Dealer = 军火商
- Streamer = 网络主播
- Cyborg = 赛博格
- Glutton = 暴食
- Jack = 杰克
- Lich = 巫妖
- Apprentice = 学徒
- Creature = 神秘生物
- Fisherman = 渔夫
- Golem = 魔像
- King = 国王
- Renegade = 叛徒
- One-Armed = 独臂
- Bull = 公牛
- Soldier = 士兵
- Masochist = 受虐狂
- Knight = 骑士
- Demon = 恶魔
- Baby = 宝宝
- Vagabond = 流浪汉
- Artificer = 技工
- Druid = 德鲁伊
- Diver = 潜水员
- Buccaneer = 海盗
- Chef = 厨师
- Builder = 建造者
- Captain = 船长
- Romantic = 浪漫之人
- Hiker = 徒步旅行者
- Cryptid = 生物
- Dwarf = 矮人
- Gangster = 匪徒
- Sailor = 水手
- Ogre = 食人魔
- Vampire = 吸血鬼
- Curious = 好奇之人

## 属性

- Damage = 伤害
- Melee Damage = 近战伤害
- Ranged Damage = 远程伤害
- Elemental Damage = 元素伤害
- Engineering = 工程学
- Attack Speed = 攻击速度
- Crit Chance = 暴击率
- Dodge = 闪避
- Armor = 护甲
- Speed = 速度
- Range = 范围
- Max HP = 最大生命值
- HP Regeneration = 生命再生
- Life Steal = 生命窃取
- Harvesting = 收获
- Luck = 幸运
- Explosion Damage = 爆炸伤害
- XP Gain = 经验获取
- Curse = 诅咒
- Materials = 材料
- HP = 生命值
- Boss = Boss
- Elite = 精英怪
- Build = 配装 / 路线
- Early = 前期
- Mid = 中期
- Late = 后期
- DPS = 每秒伤害
- AOE = 范围伤害

## 常见武器

- SMG = 冲锋枪
- Shotgun = 双管霰弹枪
- Double Barrel Shotgun = 双管霰弹枪
- Revolver = 左轮手枪
- Pistol = 手枪
- Laser Gun = 激光枪
- Wrench = 扳手
- Crossbow = 十字弓
- Slingshot = 弹弓
- Taser = 电击枪
- Sickle = 镰刀
- Ghost Scepter = 幽魂节杖
- Ghost Axe = 幽魂斧
- Ghost Flint = 幽魂燧石
- Shredder = 粉碎者
- Rocket Launcher = 火箭筒
- Grenade Launcher = 榴弹发射器
- Nuclear Launcher = 核弹发射器
- Obliterator = 毁灭者
- Shuriken = 手里剑
- Knife = 小刀
- Fireball = 火球
- Minigun = 加特林机枪
- Spear = 矛
- Rock = 石头
- Torch = 火把
- Power Fist = 强拳
- Chopper = 砍刀
- Stick = 树枝
- Plank = 木板
- Thief Dagger = 盗贼匕首
- Hammer = 铁锤
- Anchor = 船锚
- Blunderbuss = 老式火枪
- Harpoon Gun = 鱼叉枪
- Jousting Lance = 长矛
- Screwdriver = 螺丝刀
- Chainsaw = 电锯
- Icicle = 冰锥
- Pruner = 修枝剪刀
- Lightning Shiv = 雷光刀
- Thunder Sword = 雷剑
- Spoon = 勺子
- Sniper Gun = 狙击手枪
- Particle Accelerator = 粒子加速器
- Plasma Sledge = 等离子大锤
- Plasma Sledgehammer = 等离子大锤
- Potato Thrower = 土豆发射器

## 常见道具与结构

- Pocket Factory = 口袋工厂
- Laser Turret = 激光炮塔
- Landmines = 地雷
- Medical Turret = 医疗炮塔
- Giant Belt = 巨型带
- Scared Sausage = 害怕的香肠
- Bag = 背包
- Snake = 蛇
- Campfire = 篝火
- Candle = 蜡烛
- Plastic Explosive = 塑性炸药
- Ball and Chain = 流星锤
- Fish Hook = 鱼钩
- Garden = 花园
- Tree = 树木
- Extra Stomach = 另一个胃
- Cyberball = 赛博球
- Electronic Ball = 赛博球
- Weird Food = 奇怪的食物
- Rip and Tear = 狂怒
- Spicy Sauce = 辣酱
- Explosive Turret = 爆炸炮塔
- Incendiary Turret = 燃烧炮塔
- Greek Fire = 希腊火
- Barnacle = 藤壶
- Power Generator = 发电机
- Jerky = 肉干
- Decomposing Flesh = 腐烂血肉
- Frozen Heart = 冰冻之心
- Corrupted Shard = 腐化碎片
- Metal Detector = 金属探测器
- Lucky Charm = 护身符
- Coupon = 优惠券
- Recycling Machine = 回收装置
- Sharp Bullet = 尖头子弹
- Cacti Club = 刺棍
- Doc Moth = 蛾医生
- Ice Cube = 冰块
- Builder's Turret = 建造者炮台
- Alien Worm = 异形虫
- Dangerous Bunny = 危险的兔子
- Piggy Bank = 猪猪存钱罐
- Padding = 护垫
- Statue = 雕像
- Coral = 珊瑚
- Silver Bullet = 银质子弹
- Focus = 焦点
- Crown = 王冠

## 旧译名与别名归一

- `长矛` 是歧义词：旧版中文攻略常用它指 `Spear`，这时归一为 `矛`；只有英文原词为 `Jousting Lance`，或上下文明示该武器按速度成长时，才保留为 `长矛`。
- 攻速 = 攻击速度
- 射程 = 范围
- 最大生命 = 最大生命值
- 霰弹枪 = 双管霰弹枪
- 徒步者 = 徒步旅行者
- 黑帮 = 匪徒
- 幽灵杖 = 幽魂节杖
- 幽灵法杖 = 幽魂节杖
- 幽灵斧 = 幽魂斧
- 幽灵燧石 = 幽魂燧石
- 湮灭者 = 毁灭者
- 加特林 = 加特林机枪
- 动力拳套 = 强拳
- 修枝刀 = 砍刀
- 棍子 = 树枝
- 锤子 = 铁锤
- 骑枪 = 长矛
- 狙击枪 = 狙击手枪
- 激光炮台 = 激光炮塔
- 医疗炮台 = 医疗炮塔
- 巨人腰带 = 巨型带
- 恐怖香肠 = 害怕的香肠
- 营火 = 篝火
- 塑胶炸药 = 塑性炸药
- 菜园 = 花园
- 树 = 树木
- 额外胃袋 = 另一个胃
- 怪异食物 = 奇怪的食物
- 撕裂 = 狂怒
- 爆炸炮台 = 爆炸炮塔
- 燃烧炮台 = 燃烧炮塔
- 幸运符 = 护身符
- 回收机 = 回收装置
- 外星蠕虫 = 异形虫
- 危险兔子 = 危险的兔子
- 存钱罐 = 猪猪存钱罐
- 填充物 = 护垫
- 银色子弹 = 银质子弹
- 专注 = 焦点
- 闪电匕首 = 雷光刀
- 雷光匕首 = 雷光刀
- 雷霆剑 = 雷剑
- 雷霆刀 = 雷剑
- 修剪枝 = 修枝剪刀
- 修剪者 = 修枝剪刀
- 生命偷取 = 生命窃取
- 吸血 = 生命窃取
- 电子球 = 赛博球
- 能量发生器 = 发电机
- 发动机 = 发电机
- 卡特林机枪 = 猫特林机枪
- 机器人地雷 = 布雷机器人
- 烈焰使者 = 焰蜥蜴
- 掠夺蠕虫 = 搜刮冲冲
- 果冻护盾 = 水母盾

## 输出约定

- 默认输出中文名。
- 如果英文原词对识别 build 很重要，可写成 `中文（English）`，但只在第一次出现时这么写。
- 同一段里不要中英混用同一个术语，例如不要同时写“SMG”和“冲锋枪”。
- 如果资料里出现中文旧译名，也要归一到这里的当前写法，不要把旧译名直接带到最终回答里。
